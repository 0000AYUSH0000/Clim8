import 'package:clim8/pages/search_current_weather.dart';
...
class _SearchPageState extends State<SearchPage> {
  ...
  void _searchCities(String query) async {
    final response = await http.get(
      Uri.parse('https://api.openweathermap.org/data/2.5/find?q=$query&type=like&appid=$apiKey'),
    );
  ...
  void _getCurrentWeather(String cityName) async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => SearchCurrentWeather(cityWeather: _currentWeather!),
        ),
      );
    } else {
      setState(() {
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  title: Text(
                    city['name'],
                    style: const TextStyle(color: Colors.white, fontFamily: 'RedditMono'),
                  ),
                  subtitle: Text(
                    city['country'],
